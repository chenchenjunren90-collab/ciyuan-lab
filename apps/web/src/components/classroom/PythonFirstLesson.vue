...
.message-list { max-height: 410px; min-height: 340px; overflow: auto; padding: 16px; display: flex; flex-direction: column; gap: 12px; }.message-list article { display: grid; grid-template-columns: 31px 1fr; align-items: start; gap: 9px; animation: message-enter .4s cubic-bezier(.2,.8,.25,1) both; }.message-list article > i { width: 31px; height: 31px; display: grid; place-items: center; border-radius: 9px; color: #fff; font: normal 700 12px serif; }.message-list article > div { padding: 10px 11px; border: 1px solid #eee3e2; border-radius: 4px 13px 13px; background: #fff; box-shadow: 0 5px 16px #3c1c2508; }.message-list article b { color: #7b6069; font-size: 12px; }.message-list article p { margin: 5px 0 0; color: #4e4548; font-size: 12px; line-height: 1.7; white-space: pre-wrap; }.message-list article.role-student { grid-template-columns: 1fr 31px; }.message-list article.role-student > i { grid-column: 2; }.message-list article.role-student > div { grid-column: 1; grid-row: 1; border-color: #f0cdd2; border-radius: 13px 4px 13px 13px; background: #fff4f4; }
...
@keyframes message-enter { from { opacity: 0; transform: translateY(10px) scale(.985); } to { opacity: 1; transform: none; } }
.teacher-portrait i { animation: portrait-breathe 4.2s ease-in-out infinite; }
@keyframes portrait-breathe {
  0%, 100% { box-shadow: 0 9px 22px #8d10282c, 0 0 0 0 color-mix(in srgb, var(--accent) 0%, transparent); }
  50% { box-shadow: 0 9px 26px #8d102840, 0 0 0 7px color-mix(in srgb, var(--accent) 16%, transparent); }
}
html[data-motion="reduced"] :is(.message-list article, .teacher-portrait i) { animation: none; }
